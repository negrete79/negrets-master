...
const MiniPDF = (() => {
  ...
  class Doc {
    ...
    imageFit(dataUrl, x, y, bw, bh){
      ...
      this.image(dataUrl, x + (bw - dim.w * s) / 2, y + (bh - dim.h * s) / 2, dim.w * s, dim.h * s);
    }
    ...
    build(){
      ...
      this.images.forEach((im) => {
        addStream(`<< /Type /XObject /Subtype /Image /Width ${im.w} /Height ${im.h} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${im.data.length} >>`, im.data);
      });

      this.pages.forEach((pg, i) => {
        const content = bytesOf(pg.ops.join('\n'));
        addStream(`<< /Length ${content.length} >>`, content);
        let xobj = '';
        if (pg.imgs.length){
          xobj = ' /XObject << ' + pg.imgs.map((nm) =>
            `/${nm} ${firstImg + this.images.findIndex((m) => m.name === nm)} 0 R`).join(' ') + ' >>';
        }
        addStr(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${this.W} ${this.H}] /Resources << /Font << /F1 3 0 R /F2 4 0 R >>${xobj} >> /Contents ${firstContent + i*2} 0 R >>`);
      });
